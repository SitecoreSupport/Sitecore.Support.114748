    ...
    public class AutoMapper : IAutoMapper<IFormModel, FormViewModel>
    {
        public FormViewModel GetView(IFormModel formModel)
        {
            ...
            FormViewModel formViewModel = new FormViewModel()
            {
                ...
                SuccessSubmit = false,
                Errors =
                    formModel.Failures.Select<ExecuteResult.Failure, string>(
                        (Func<ExecuteResult.Failure, string>) (x => x.ErrorMessage)).ToList<string>(),
                Visible = true,
                ...
                ReadQueryString = (formModel as Sitecore.Forms.Mvc.Models.FormModel).ReadQueryString,
                QueryParameters = (formModel as Sitecore.Forms.Mvc.Models.FormModel).QueryParameters
            };
            formViewModel.CssClass =
                ((formModel.Item.FormTypeClass ?? string.Empty) + " " + (formModel.Item.CustomCss ?? string.Empty) + " " +
                 (formModel.Item.FormAlignment ?? string.Empty)).Trim();
            ReflectionUtils.SetXmlProperties((object) formViewModel, formModel.Item.Parameters, true);
            formViewModel.Sections =
                ((IEnumerable<Item>) formModel.Item.SectionItems).Select<Item, SectionViewModel>(
                    (Func<Item, SectionViewModel>) (x => this.GetSectionViewModel(new SectionItem(x), formViewModel)))
                    .Where<SectionViewModel>((Func<SectionViewModel, bool>) (x => x != null))
        ...
        public void SetModelResults(FormViewModel view, IFormModel formModel)
        {
            ...
            List<ControlResult> list =
                view.Sections.SelectMany<SectionViewModel, FieldViewModel>(
                    (Func<SectionViewModel, IEnumerable<FieldViewModel>>) (x => (IEnumerable<FieldViewModel>) x.Fields))
                    .Select<FieldViewModel, ControlResult>(
                        (Func<FieldViewModel, ControlResult>) (x => ((IFieldResult) x).GetResult()))
                    .Where<ControlResult>((Func<ControlResult, bool>) (x => x != null))
        ...
        protected SectionViewModel GetSectionViewModel(SectionItem item, FormViewModel formViewModel)
        {
            ...
            sectionViewModel.Fields =
                ((IEnumerable<FieldItem>) item.Fields).Select<FieldItem, FieldViewModel>(
                    (Func<FieldItem, FieldViewModel>) (x => this.GetFieldViewModel((IFieldItem) x, formViewModel)))
                    .Where<FieldViewModel>((Func<FieldViewModel, bool>) (x => x != null))
                    .ToList<FieldViewModel>();
        ...
        protected FieldViewModel GetFieldViewModel(IFieldItem item, FormViewModel formViewModel)
        {
            ...
            if (formViewModel.ReadQueryString && formViewModel.QueryParameters != null &&
                !string.IsNullOrEmpty(formViewModel.QueryParameters[fieldViewModel.Item.Name]))
            {
