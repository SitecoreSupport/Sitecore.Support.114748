...
namespace Sitecore.Support.Forms.Mvc.Services
{
    public class AutoMapper : IAutoMapper<IFormModel, FormViewModel>
    {
        public FormViewModel GetView(IFormModel formModel)
        {
            ...
            FormViewModel formViewModel = new FormViewModel() { UniqueId = formModel.UniqueId, Information = formModel.Item.Introduction ?? string.Empty, IsAjaxForm = formModel.Item.IsAjaxMvcForm, IsSaveFormDataToStorage = formModel.Item.IsSaveFormDataToStorage, Title = formModel.Item.FormName ?? string.Empty, Name = formModel.Item.FormName ?? string.Empty, TitleTag = formModel.Item.TitleTag.ToString(), ShowTitle = formModel.Item.ShowTitle, ShowFooter = formModel.Item.ShowFooter, ShowInformation = formModel.Item.ShowIntroduction, SubmitButtonName = formModel.Item.SubmitName ?? string.Empty, SubmitButtonPosition = formModel.Item.SubmitButtonPosition ?? string.Empty, SubmitButtonSize = formModel.Item.SubmitButtonSize ?? string.Empty, SubmitButtonType = formModel.Item.SubmitButtonType ?? string.Empty, SuccessMessage = formModel.Item.SuccessMessage ?? string.Empty, SuccessSubmit = false, Errors = formModel.Failures.Select<ExecuteResult.Failure, string>((Func<ExecuteResult.Failure, string>)(x => x.ErrorMessage)).ToList<string>(), Visible = true, LeftColumnStyle = formModel.Item.LeftColumnStyle, RightColumnStyle = formModel.Item.RightColumnStyle, Footer = formModel.Item.Footer, Item = formModel.Item.InnerItem, FormType = formModel.Item.FormType, ReadQueryString = formModel.ReadQueryString, QueryParameters = formModel.QueryParameters };
        ...
        protected SectionViewModel GetSectionViewModel(SectionItem item, FormViewModel formViewModel)
        {
            ...
            SectionViewModel sectionViewModel = new SectionViewModel() { Fields = new List<FieldViewModel>(), Item = item.InnerItem };
        ...
        protected FieldViewModel GetFieldViewModel(IFieldItem item, FormViewModel formViewModel)
        {
            ...
            if (formViewModel.ReadQueryString && formViewModel.QueryParameters != null && !string.IsNullOrEmpty(formViewModel.QueryParameters[fieldViewModel.Item.Name]))
            {
                ...
                if (method != (MethodInfo)null)
                    ...
                    {
            (object) formViewModel.QueryParameters[fieldViewModel.Item.Name]
                    });
            }
